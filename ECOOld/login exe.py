...
async def auth_web(request: Request):
    ...
    if request.method == 'POST':
        ...
        if LDAP_AUTH(username, password):
            ...
            response.set_cookie(key='message', value="Entraste, chaval")
    ...
    return templates.TemplateResponse("loginWeb.html", {"request": request, "error_message": error_message})
...
async def protected_web(request: Request):
    ...
    if session_id and username:
        ...
        if session:
            return HTMLResponse(content=f"<h1>Hola, {username}!</h1>{message}")
        else:
