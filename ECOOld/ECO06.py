...
@app.get('/')
def menu(rpta: Request):
    return templates.TemplateResponse('eco.html', {'request': rpta})
...
@app.get('/EnsenaDatos', response_class=HTMLResponse)
def ensena (): 
    ...
    if pool is not None:
        ...
        try:   
            entorno = Environment(loader=FileSystemLoader('templates2'))

            template = entorno.get_template('eco02.html')
            ...
            for r in registros:   
                ...
                s=f"select bdoc, nom_doc from ecodocumentos_tmp where nig='{r[0]}'"
                cursor.execute(s)
                ...
                for r2 in reg2:
                    ...
                    if r2[1][-3:].upper()=='TXT':
                        ...
                        docto = {
                                'doc':base64.b64encode(elto).decode('utf-8'),
                                'nomdoc':r2[1]
                                }
